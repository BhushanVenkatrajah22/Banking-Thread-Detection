...
export default function DigitalTwinDetailsPage() {
  ...
  const driftHistoryData = employee.weeklyPattern.map((risk, index) => {
  ...
  // Let's customize base on employee risk. If Rajesh (EMP001), high off-hours activity.
  const isHighRisk = employee.predictionRisk >= 70;
  
  const hourlyActivityData = Array.from({ length: 24 }).map((_, hour) => {
