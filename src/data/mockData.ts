...
export const getLiveActivities = () => {
  return [
    { id: 1, time: 'Just now', type: 'usb', name: 'Rajesh Kumar', msg: 'Connected Cruzer USB flash storage', severity: 'high' },
    { id: 2, time: '5 mins ago', type: 'file', name: 'Rajesh Kumar', msg: 'Downloaded 1,450 Treasury Ledger Excel files', severity: 'high' },
    { id: 3, time: '12 mins ago', type: 'privilege', name: 'Priya Sharma', msg: 'Bypassed sudo admin verification for production SQL', severity: 'high' },
    { id: 4, time: '25 mins ago', type: 'network', name: 'Priya Sharma', msg: 'Terminated syslogd logs service daemon', severity: 'high' },
    { id: 5, time: '1 hour ago', type: 'database', name: 'Amit Patel', msg: 'Queried dormant account ledger balance details', severity: 'medium' },
    { id: 6, time: '2 hours ago', type: 'login', name: 'Rajesh Kumar', msg: 'VPN Login from unusual subnet IP 103.45.12.89', severity: 'medium' },
    { id: 7, time: '3 hours ago', type: 'login', name: 'Ananya Rao', msg: 'Standard remote VPN session established successfully', severity: 'low' },
    { id: 8, time: '4 hours ago', type: 'database', name: 'Sanjay Mehta', msg: 'Began query sequence on Wealth Management assets', severity: 'low' }
  ];
};
...
export const copilotInference = (message: string, employeesList: Employee[], predictionsList: Prediction[]): { response: string; references?: string[] } => {
  ...
  if (query.includes('rajesh') || query.includes('kumar') || query.includes('emp001')) {
    return {
      response: `**Rajesh Kumar (Fixed Income Trader, Treasury & Markets) - Insider Threat Profile:**

* **Current Status:** High Risk Alert (92% Risk Probability).
* **AI Twin Baseline Deviations:**
  * **Working Hours:** Rajesh normally logs in between 08:30 AM and 17:30 PM. Today, he logged in via remote VPN at **03:15 AM** (78% working hours drift).
...
**Recommended Actions:**
1. Temporarily freeze Rajesh Kumar's Active Directory and trading credentials.
...
3. Schedule an immediate executive security audit review with the Treasury compliance director.`,
      references: ['Rajesh Kumar Profile', 'Case CASE-2026-001', 'Threat Prediction PRD001']
    };
  }
  
  if (query.includes('priya') || query.includes('sharma') || query.includes('emp002')) {
    return {
      response: `**Priya Sharma (Database Administrator, IT & Cybersecurity) - Insider Threat Profile:**

* **Current Status:** Critical Alert (84% Risk Probability).
* **AI Twin Baseline Deviations:**
  * **Privilege Escalation:** Priya requested emergency administrative bypass on server \`DB-PROD-RETAIL-01\` outside standard Change Advisory Board (CAB) windows.
...
3. Automatically restart the database logging daemon remotely.`,
      references: ['Priya Sharma Profile', 'Case CASE-2026-002', 'Threat Prediction PRD002']
    };
  }
...
1. **Rajesh Kumar (Treasury)**: 92% probability of *Data Theft & Exfiltration*. Active exfiltration on USB detected.
2. **Priya Sharma (IT & Security)**: 84% probability of *Privilege Abuse & Security Sabotage*. Database logging service was manual disabled.
3. **Amit Patel (Retail Banking)**: 76% probability of *Financial Fraud & Procedural Overrides*. Attempting account limits modifications on dormant portfolios.
...
1. **Deploy Auto-Isolation Rules:** For employees exceeding 85% prediction risk with active exfiltration indicators (e.g., Rajesh Kumar), immediately trigger temporary endpoint network containment.
...
3. **Dual-Authorization Enforcement:** Restrict Branch Managers (e.g., Amit Patel) from modifying transaction limits or viewing dormant accounts without a secondary Compliance Officer approval.`,
